...
class BasicSSHUploadStrategy extends UploadStrategy {
    ...
    async _sendCode(config) {
        ...
        command = this._getShhCommand(config) + ` ${outterQuot}if [ -x ${innerQuot}$(command -v docker)${innerQuot} ]; then cd ${remoteRepoPath}/deploy && docker-compose down -v; fi${outterQuot}`;
        await executeCommand(command);
    ...
    async _configureInstance(config) {
        ...
        command = this._getShhCommand(config) + " " + outterQuot + "sudo add-apt-repository " + innerQuot + "deb [arch=amd64] https://download.docker.com/linux/debian buster stable" + innerQuot + outterQuot;
        await executeCommand(command);
